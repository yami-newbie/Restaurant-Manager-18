...
function BookTable() {
  ...
  useEffect(() => {
    if (day) getAllOrderByDate(day.toLocaleDateString());
  }, [day]);
  ...
  const getOrderPerTableByDay = (id_Ban) => {
    if (orders) {
      ...
      if (order && order.length > 0) return datban.getDatBanByID_DatBan(order);
    }
  };
  ...
  const handleConfirm = () => {
    datban
      .addDatBan({ time: time, day: day.toLocaleDateString() })
      .then((data) => {
        selected.map((sl) => {
